...
export (SpriteFrames) var bg_sprites
...
func _ready():
	...
	bg.set_sprite_frames(bg_sprites)
...
#		to make the effect disapear.
		...
		set_size(Vector2(screen_res.size.x-600,screen_res.size.y/dialog_frame_height-100))
		...
		if position==0:
			...
			set_pos(Vector2(300,screen_res.size.y-(get_size().y)-100))
		var size = get_size().x
		face_v_pos = get_size().y - (texture_height/2)+100 
		face_h_pos = get_size().x/2 - (texture_width/4)
		background_pos = Vector2(-screen_res.size.x/2 + (screen_res.size.x - get_size().x), -screen_res.size.y + (screen_res.size.y - get_size().y))
		bg.set_global_pos(Vector2(0,0))
		
		if show_debug_messages:
			print("FACE V POS ", face_v_pos)
			print("FACE H POS ", face_h_pos)
			print("SCREENSIZE ", screen_res.size.x/2)
		if side == 0:
...
#			textObj.set_margin(2, dimensions.text_margin.right)
			face.set_pos(Vector2(face_h_pos ,face_v_pos))
